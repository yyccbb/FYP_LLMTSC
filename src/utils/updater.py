...
class Updater:
    ...
    def load_sample_for_agents(self):
        ...
        if self.dic_traffic_env_conf['MODEL'] in ["EfficientPressLight",  "EfficientMPLight",
        ...
        elif self.dic_traffic_env_conf['MODEL'] in ["PressLight"]:
            for i in range(self.dic_traffic_env_conf['NUM_INTERSECTIONS']):
                sample_set = self.load_sample_with_forget(i)
                self.agents[i].prepare_Xs_Y(sample_set)
        elif self.dic_traffic_env_conf['MODEL'] in ["EfficientColight", "AdvancedColight"]:
            samples_list = []
            for i in range(self.dic_traffic_env_conf['NUM_INTERSECTIONS']):
                sample_set = self.load_sample_with_forget(i)
                # [sample1, sample2, ...]
