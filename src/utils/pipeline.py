...
from src.utils.model_test import model_test
from src.utils.generator import Generator
from src.utils.updater import Updater
from src.utils.construct_sample import ConstructSample
...
def path_check(dic_path):
    if os.path.exists(dic_path["PATH_TO_WORK_DIRECTORY"]):
        if dic_path["PATH_TO_WORK_DIRECTORY"] != "logs/default":
            raise FileExistsError
        else:
            pass
    else:
        os.makedirs(dic_path["PATH_TO_WORK_DIRECTORY"])
    if os.path.exists(dic_path["PATH_TO_TRAINED_CHECKPOINTS"]):
        if dic_path["PATH_TO_TRAINED_CHECKPOINTS"] != "model/default":
            raise FileExistsError
        else:
            pass
    else:
        os.makedirs(dic_path["PATH_TO_TRAINED_CHECKPOINTS"])
...
class Pipeline:
    ...
    def run(self, run_name, multi_process=False):
        ...
        for cnt_round in range(self.dic_traffic_env_conf["NUM_ROUNDS"]):
            ...
            if self.dic_traffic_env_conf["MODEL"] in self.dic_traffic_env_conf["LIST_MODEL_NEED_TO_UPDATE"]:
                if multi_process:
            ...
            results, state_action_log = model_test(self.dic_path["PATH_TO_TRAINED_CHECKPOINTS"], self.dic_path["PATH_TO_DATA"],
                                                        cnt_round,
