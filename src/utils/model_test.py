...
def model_test(model_dir, data_dir, cnt_round, run_cnt, _dic_traffic_env_conf, logger):
    dic_traffic_env_conf = deepcopy(_dic_traffic_env_conf)
    logs_dir = model_dir.replace("checkpoints", "logs")
    model_round = "round_%d" % cnt_round
    dic_path = {"PATH_TO_DATA": data_dir, "PATH_TO_TRAINED_CHECKPOINTS": model_dir, "PATH_TO_WORK_DIRECTORY": logs_dir}
    with open(os.path.join(logs_dir, "agent.conf"), "r") as f:
        dic_agent_conf = json.load(f)
    if os.path.exists(os.path.join(logs_dir, "anon_env.conf")):
        with open(os.path.join(logs_dir, "anon_env.conf"), "r") as f:
            dic_traffic_env_conf = json.load(f)
    dic_traffic_env_conf["RUN_COUNTS"] = run_cnt

    if dic_traffic_env_conf["MODEL"] in dic_traffic_env_conf["LIST_MODEL_NEED_TO_UPDATE"]:
