...
class CoLightAgent(Agent):
    def __init__(self, dic_agent_conf=None, dic_traffic_env_conf=None, dic_path=None, cnt_round=None,
        ...
        if cnt_round == 0:
            ...
            if os.listdir(self.dic_path["PATH_TO_TRAINED_CHECKPOINTS"]):
                self.q_network.load_weights(
                    os.path.join(self.dic_path["PATH_TO_TRAINED_CHECKPOINTS"], "round_0_inter_{0}.h5".format(intersection_id)),
                    by_name=True)
    ...
    def save_network(self, file_name):
        self.q_network.save(os.path.join(self.dic_path["PATH_TO_TRAINED_CHECKPOINTS"], "%s.h5" % file_name))

    def save_network_bar(self, file_name):
        self.q_network_bar.save(os.path.join(self.dic_path["PATH_TO_TRAINED_CHECKPOINTS"], "%s.h5" % file_name))
