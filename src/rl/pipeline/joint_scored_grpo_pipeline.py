...
from src.utils.config import DIC_CITY_ALIASES, DIC_CITY_SPECS, DIC_TRAFFIC_ENV_CONF
...
class JointScoredGRPOPipeline:
    ...
    def __init__(
        self,
        config: Dict[str, Any],
        collector: Optional[JointScoredRolloutCollector] = None,
    ...
    def _finalize_config_defaults(self) -> None:
        ...
        rollout_cfg.setdefault("k", 8)
        rollout_cfg.setdefault("invalid_action_reward", 0.0)
        rollout_cfg.setdefault("horizon", 3)
        rollout_cfg.setdefault("discount_gamma", 0.99)
        rollout_cfg.setdefault("reward_mode", "total_incoming_queue_length")
        rollout_cfg.setdefault("continuation_policy", "resample_each_step")

        reward_mix = rollout_cfg.setdefault("reward_mix", {})
        reward_mix.setdefault("alpha", 0.6)
        reward_mix.setdefault("beta", 0.3)
        reward_mix.setdefault(
            "global_weight",
            1.0 - float(reward_mix.get("alpha", 0.6)) - float(reward_mix.get("beta", 0.3)),
        )
    ...
    def _build_city_and_env_conf(self) -> None:
        ...
        if not raw_city:
            raise ValueError("City is not provided.")
        ...
        env_extra = {
            ...
            "MODEL_NAME": env_cfg.get("model_name", "JointScoredGRPO"),
            "MODEL": env_cfg.get("model_name", "JointScoredGRPO"),
            "PROJECT_NAME": env_cfg.get("project_name", "JointScoredGRPO"),
    ...
    def _init_q_rewarder(self) -> Any:
        return None

    def _init_collector(self) -> JointScoredRolloutCollector:
        rollout_cfg = self.config["rollout"]
        return JointScoredRolloutCollector(
            env=self.env,
            tokenizer=self.tokenizer,
            model=self.policy_model,
            k=int(rollout_cfg.get("k", 8)),
            invalid_action_reward=float(rollout_cfg.get("invalid_action_reward", 0.0)),
            horizon=int(rollout_cfg.get("horizon", 3)),
            discount_gamma=float(rollout_cfg.get("discount_gamma", 0.99)),
            reward_mode=str(rollout_cfg.get("reward_mode", "total_incoming_queue_length")),
            reward_mix=dict(rollout_cfg.get("reward_mix", {})),
            continuation_policy=str(rollout_cfg.get("continuation_policy", "resample_each_step")),
    ...
    def _resolve_grpo_output_dir(self, configured_output_dir: Optional[str]) -> str:
        checkpoint_root = self.config["paths"].get("checkpoint_root", "checkpoints/rl_grpo_joint_scored")
